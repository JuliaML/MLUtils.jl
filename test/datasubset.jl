...
@testset "DataSubset constructor" begin
    ...
    #     @test numobs(DataSubset(X, obsdim = 1)) == 4
    #     @test numobs(DataSubset(X, 1:3, obsdim = 1)) == 3
...
#             @test @inferred(numobs(subset)) == numobs(subset[1]) == numobs(subset[2]) == 50
...
#         @test_throws DimensionMismatch datasubset((X,X), 1:150)
#         @test_throws DimensionMismatch datasubset((X,X), 1:150, (ObsDim.Last(),))
#         @test_throws DimensionMismatch datasubset((X,X))
