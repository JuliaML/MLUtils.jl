...
function BatchView(data::T; size::Int=1, partial::Bool=true) where {T}
    ...
    E = typeof(_datasubset(data, 1:size))
...
Base.getindex(A::BatchView, batchindex::Int) =
    _datasubset(A.data, _batchrange(A, batchindex))
...
function Base.getindex(A::BatchView, batchindices::AbstractVector)
    ...
    BatchView(_datasubset(A.data, obsindices); A.size, A.partial)
end

_datasubset(A::BatchView) = A
_datasubset(A::BatchView, i) = A[i]
